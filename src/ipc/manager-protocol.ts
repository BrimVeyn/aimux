...
export type ManagerRequest =
  ...
  | { id: string; type: 'serializeBuffer'; payload: { projectId: string; tabId: string } }
...
export type ManagerEvent =
  ...
  // Additive (capability `tabBytes`): the raw PTY byte stream the GUI host
  // feeds to xterm.js. A TM that never sends it costs the TUI nothing.
  | {
      type: 'tabBytes'
      payload: { projectId: string; tabId: string; data: string }
    }
...
export function parseManagerRequest(value: unknown): ManagerRequest {
  ...
  switch (value.type) {
    ...
    case 'serializeBuffer':
      assert(isString(value.payload.projectId), 'serializeBuffer.projectId must be a string')
...
export function parseManagerMessage(value: unknown): ManagerResponse | ManagerEvent {
  ...
  switch (value.type) {
    ...
    case 'tabBytes':
      assert(isString(value.payload.projectId), 'tabBytes.projectId must be a string')
