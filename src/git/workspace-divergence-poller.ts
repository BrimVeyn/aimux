...
/**
 * Headless divergence poller. Returns a disposer. The GUI host has no React
 * root, so it drives the same loop directly; the hook below is the TUI's.
 */
export function startWorkspaceDivergencePolling(enabled: boolean): () => void {
  if (!enabled) return () => {}
...
export function useWorkspaceDivergencePolling(enabled: boolean): void {
  useEffect(() => startWorkspaceDivergencePolling(enabled), [enabled])
}
