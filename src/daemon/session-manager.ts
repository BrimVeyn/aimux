...
export class SessionManager extends EventEmitter<SessionManagerEvents> {
  ...
  private getOrCreateRegistry(projectId: string): SessionRegistry {
    ...
    registry.on('bytes', (tabId, data) => {
      this.emit('bytes', projectId, tabId, data)
    })
  ...
  /** Raw PTY bytes for the GUI host's xterm.js client; the TUI never calls it. */
  serializeBuffer(projectId: string, tabId: string): string {
    return this.registries.get(projectId)?.serializeBuffer(tabId) ?? ''
  }
