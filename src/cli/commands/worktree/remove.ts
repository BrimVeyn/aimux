...
export const worktreeRemove: CliCommand = {
  ...
  run: async (ctx) => {
    ...
    const worktree =
      project.worktrees?.find((w) => w.id === target) ??
      project.worktrees?.find((w) => w.path === target)
    if (!worktree) {
      const known = project.worktrees?.map((w) => w.id).join(', ') ?? '(none)'
    ...
    if (!primary) {
      throw new Error('project has no primary worktree — cannot resolve repoRoot for git remove')
    }
  ...
  summary: 'Remove a worktree from the active project',
  verb: 'remove',
}
