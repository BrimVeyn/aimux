...
export class TerminalManagerClient extends EventEmitter<ManagerClientEvents> {
   ...
   * Requires manager protocol v4+ (enforced by MANAGER_PROTOCOL_MIN_VERSION):
   * a v3 TM would have failed handshake before this method could be called,
   * so by the time we get here the TM understands `setBroadcastEnabled`.
   */
