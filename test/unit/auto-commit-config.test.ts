...
test('autoCommit default is opt-out with cheap models', () => {
...
test('user can disable autoCommit', () => {
  const resolved = resolveConfig(defineConfig({ autoCommit: { enabled: false } }))
