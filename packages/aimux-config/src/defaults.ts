  ...
  const kb = new KeymapBuilder()
    ...
    .mode('terminal-input', (m) =>
      m
        ...
        .map('<Leader>H', actions.resizePane(-1, 'vertical'), 'Shrink ←')
        .map('<Leader>L', actions.resizePane(1, 'vertical'), 'Grow →')
        .map('<Leader>K', actions.resizePane(-1, 'horizontal'), 'Shrink ↑')
        .map('<Leader>J', actions.resizePane(1, 'horizontal'), 'Grow ↓')
